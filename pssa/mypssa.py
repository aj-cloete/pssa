...
try:
    ...
    print(f"Using Seaborn ({seaborn.version})")
...
class mySSA(object):
    ...
    def get_contributions(X=None, s=None, plot=True):
        """Calculate the relative contribution of each of the singular
        values."""
    ...
    def diagonal_averaging(hankel_matrix):
        ...
        # to deal with nulls, start with `np.zeros((L,K))`
        if L > K:
            mat = mat.T
        ret = []
    ...
    def decompose(self, verbose=False):
        """Perform the Singular Value Decomposition and identify the rank of
        the embedding subspace.

        Characteristic of projection: the proportion of variance
        captured in the subspace.
        """
        X = self.X_com
        self.S = X * X.T
    ...
    def view_s_contributions(
        ...
        """View the contribution to variance of each singular value and its
    ...
    def view_reconstruction(
        ...
        """Visualise the reconstruction of the hankel matrix/matrices passed
        to.
    ...
    def forecast_recurrent(
        ...
        **plotargs,
        ...
        using recurrent methodology This method also fills any missing data
